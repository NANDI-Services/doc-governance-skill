...
- **Fixtures de test deben neutralizar el git global** (2026-08): los self-tests arman repos en tmpdir con `git init` + commits, y eso **hereda `core.hooksPath` global del usuario**. En la máquina donde se escribió 0.9.2 eso disparaba un rebuild de graphify por cada commit de cada fixture, que además escribía `graphify-out/` dentro del repo temporal y rompía el aserto "el dry-run no modificó nada". Todo repo de prueba debe apuntar `core.hooksPath` a un directorio inexistente (`isolateRepo()` en `bin/lib/self-test-update.js`). Regla: un fixture que hereda config global del usuario no es un fixture, es el entorno del usuario — y falla distinto en cada máquina.
