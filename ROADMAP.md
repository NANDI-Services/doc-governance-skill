...
- **Heading-context downgrade** (feedback Almacen jul-2026, item b): degradar a INFO cualquier match donde el `codeRef` aparezca en `.md` bajo una sección cuyo heading matchea `changelog|history|snapshot|completed|done|closed|graph-report|deferred|backlog`. Menciones arqueológicas (roadmap task marcada `[x]`, entry de CHANGELOG histórico) no reflejan drift real. Requiere `bin/lib/scan.js` indexar `codeRef → heading` (hoy solo indexa headings y codeRefs por doc, no la relación) y `bin/update.js` leerlo del map. Activar cuando el warning-mix real muestre >20% de menciones históricas post-v0.7.
- **`--reseal-if-clean` flag** (feedback Almacen jul-2026, item d): opt-in CLI para auto-avanzar `.doc-governance/map.md` al `HEAD` actual si el reporte cierra con `0 critical, 0 warnings`. Hoy el flujo interactivo (`SKILL.md ## Root Invocation Behavior > Flujo steady-state paso 3`) ya pregunta si resellar — el flag es la variante non-interactive para CI/hooks. Activar cuando alguien pida CI que auto-avance baseline en push a main sin gate humano.
  - Nota 0.9.0: el riesgo que hacía incómodo este flag desapareció. Antes, resellar automáticamente garantizaba drift sobre el propio commit del re-sello; con `sealed_dirty:` ese commit cierra limpio. `.github/scripts/release.sh` ya implementa el caso concreto (resellar y commitear el map en cada release) sin necesitar el flag — si alguien lo pide, mirar ahí primero.
- **`file:line:` snippets en `suggested_action`** (feedback Almacen jul-2026, item e): incluir línea + contexto ±3 del `.md` donde aparece el codeRef, formato `git grep -C 3`. Requiere pre-indexar rangos por codeRef en scan.js (aumenta `.doc-governance/map.md` ~2-3x tamaño) o hacer grep runtime por match (I/O extra por warning). Activar cuando los agents empiecen a reportar "hice grep manual del path para encontrar la sección" como fricción real.
